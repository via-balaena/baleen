...
//! is caught too, at any level: since `parent`, `child`, `writable`, and `leaf` are all
//! fuzzer-chosen, this exercises foreign *leaves* (a data page under an `L1`), foreign
//! *superpages* (a leaf mapped directly by an `L2`/`L3` entry), and foreign *interior* node
//! shares (a table pointing at another domain's `L(k-1)` node, sharing a subtree) alike. `DomainCreate`/`DomainDestroy` are the birth and death of a whole domain,
