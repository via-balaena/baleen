...
| `mislevelled_link_preservation.rs` **(the metal refinement — Phases I-2a + I-2b)** | **`MislevelledLink` preserved, ∀-N — the whole hierarchy invariant; residual CLOSED.** The standing hierarchy invariant (every live edge's parent is a page table and its child is exactly the reference the entry's shape demands) is preserved by **every** transition at arbitrary edge, frame and level count. *I-2a — the additive half:* `p2m_link` (two `get_type` acquires **establish** the new edge, an acquire cannot flip a live type — the one borrow from `TypeConfusion` exclusivity), `grant_map`/`pin` (adds-only), `p2m_allocate` (**purely type-monotone, no borrow**), plus the base case. *I-2b — the decrement half:* the count↔holder coupling `TypeRefsAccounted` (`accounted`) — each recorded count equals the cardinality of its holder family, the page-table/writable holders **counted over the edge seq** (the `refcount_mismatch.rs` technique) and pins/grant-maps as opaque remainders — is proven a **full inductive invariant** (established at boot, preserved by every additive *and* decrement transition), and from it a *surviving* edge (still in the seq ⇒ automatically a holder) keeps its ends' types, discharging I-2a's `type_preserved_on` hypotheses with **no injective correspondence**. So `unlink`/`unlink_all`, `unpin`/`grant_unmap`, `free` and `DomainDestroy` (a loop of the primitives) are hypothesis-free. The page-table family of the coupling is additionally checked on the **real** `System` by `p2m::first_violation` (I-2b/1, the `PagetableRefsAccounted` violation, `≥`-form). *Finding: for `MislevelledLink`, `DomainDestroy`'s `has_foreign_link_into` precondition **is** load-bearing (a surviving foreign edge into a freed frame would break it) — cashing the localization `foreign_link_preservation.rs` recorded; in the proof it lands as `free`'s `refs == 0` premise.* | the metal — `docs/STAGE2-REFINEMENT-FORALL-N.md` |
...
| `noninterference_instantiation.rs` **(Tier D — closes GAP-C)** | **The concrete instantiation — discharging the meta-theorem's premises.** `noninterference_theorem.rs` proves the unwinding theorem over *uninterpreted* `obs`/`step`/`interferes`, taking `local_respect()`/`step_consistent()` as **premises**; the fragment lemmas prove pieces over independent symbols, so the composition was doc-only (GAP-C). This file defines a **concrete** `Sys`/`obs`/`step`/`interferes`, proves `local_respect()` **and** `step_consistent()` as **theorems** (`local_respect_holds`, `step_consistent_holds`), and re-runs the unwinding induction (`ni_theorem_a`/`ni_theorem_b`) **premise-free**. **All five** transition classes: creation, signal, consent + the read-closure, authority, and the **`DomainDestroy` cascade** (the multi-domain one — four `obs` components at once + the intransitive `teardown_reach`; the `map`-identity threaded as `wf` and proven preserved, `wf_step_destroy`). Forced **four** `obs` corrections: a domain observes **its own incoming authority** (`may_create[a]`, `controls[·][a]`), `obs⁺` carries `owner(frame)`, the reach term extends to the **read direction** (destroying `a`'s grantor — *later retracted by ⑦, see below*), and the actor observes **its own outgoing destroy authority** (`controls[a][·]`) — all non-vacuous (see below). **⑦ then split the single read-closed `Obs` into `Obs` (integrity) / `ObsPlus` (confidentiality)**, matching the bridge's two surfaces (design-lesson #58): `local_respect`/Theorem A stay at `obs`, `step_consistent`/Theorem B move to `obs⁺`, and the **blanket `reads_from` disjunct leaves `interferes`** — it was an artifact of the single surface, and it had made Theorem A strictly weaker than what the bridge already demonstrated. Dropping it exposed that the ②′-(c) **borrow** flow was *invisible* to `Obs` rather than absent (the old map surface carried only maps over frames `a` **owns**), so `Obs` gained `held` and `teardown_borrow` became **forced**. `interferes ⊊ interferes_pre7` is proven in-file (`interferes_shrank` + `interferes_shrank_strictly`) so the narrowing is machine-checked rather than asserted. Closes the *composition* gap; the enumerator bridge remains the tie to real code. | Tier D — non-interference |
| `device_assignment_preservation.rs` **(the SMMU arc — rung 4a)** | **The device→domain assignment relation coupled to the lifecycle, ∀-N.** `holder(dev) == Some(d) ⇒ d is Live` — the device clause of `DeadDomainReferenced` — is preserved by every transition class that can move toward violating it, over an **arbitrary device population**: `device_assign` (the mint gate refuses a non-`Live` assignee), `device_release` (monotone), `DomainCreate` (monotone in liveness), and `DomainDestroy` — the **only** class that removes liveness, and therefore the only one whose preservation is not monotonicity. Its argument *is* the teardown sweep, proven **exact** in both directions (`sweep_is_exact`): nothing names the destroyed domain, and every device held by anyone else is untouched — the second half being what no invariant would ever check, since an over-reaching sweep satisfies the property while silently disarming every other domain's devices. *Deliberately absent: "a device has at most one holder", which is a property of the `Option<DomId>` representation rather than an invariant of the code — a proof of it could not fail.* Kani (`hv-verify::device_assignment`) proves the same transitions total and exactly-scoped on the **shipped** `hv_core::device` code at bounded device count; this file is the ∀-**size** half. | the metal — `docs/SMMU-DEVICE-ASSIGNMENT.md` |
...
| `noninterference_instantiation.rs` | drop `maycreate` from `Obs` (authority-excluding `obs`, per doc §2.1) | `step_consistent_holds`: `postcondition not satisfied` (creation channel) — **the GAP-C finding: Theorem B is false for the documented `obs`** |
| `noninterference_instantiation.rs` | drop the `owner` field from `ReadCap`/`obs⁺` | `step_consistent_holds`: `postcondition not satisfied` (the confidentiality read direction) |
| `noninterference_instantiation.rs` | drop the `controllers` field from `Obs` | `step_consistent_holds`: `postcondition not satisfied` (affinity channel — same self-authority finding) |
| `noninterference_instantiation.rs` | drop `teardown_reach` from `interferes` | `local_respect_holds`: `assertion failed` at `¬reach(a, c)` (the `DomainDestroy` intransitive channel — teardown-reach is load-bearing) |
| `noninterference_instantiation.rs` | empty out `a_controls_out` (actor no longer observes its outgoing authority) | `step_consistent_holds`: `assertion failed` at the `DomainDestroy` guard-agreement — the actor's own outgoing destroy authority is unobserved, so the destroy can fire in one run and not the other |
| `noninterference_instantiation.rs` **(⑦)** | drop `teardown_borrow` from `interferes` | `local_respect_holds`: `assertion failed` at the `DomainDestroy` case — the ②′-(c) borrow flow (`b` dying force-reclaims the page `a` mapped from it) has nothing to authorize it. **Note this probe passes GREEN if you also drop `held` from `Obs`** — which is how ⑦ found that the flow was invisible rather than absent, and why `held` had to be added before the disjunct could be said to be forced. |
| `noninterference_instantiation.rs` **(⑦)** | drop **both** `held` from `Obs` *and* `teardown_borrow` from `interferes` (the pre-⑦ owner-side-only map surface) | ***no error* — 9 verified, 0 errors.** That is the point of the row: with the held-map surface gone the borrow write lands outside the observation, so a relation missing `teardown_borrow` looks perfectly sound. This pair of rows is the non-vacuity argument for `teardown_borrow`; neither row alone is. |
| `noninterference_instantiation.rs` **(⑦)** | re-add the blanket `reads_from(s, a, b)` disjunct to `interferes` (i.e. undo ⑦'s narrowing) | `interferes_shrank_strictly`: `assertion failed` at `!interferes(s, 1, 0)`. Worth noting *why* this is caught at all: re-widening a relation cannot break `local_respect_holds` (a wider `interferes` is a weaker theorem, so the probe technique used for every other row is blind to it). It is caught because the strictness **witness** is in the file as a proof obligation — so the narrowing is pinned, not merely recorded. |
| `device_assignment_preservation.rs` | drop `live(to)` from `assign_preserves` (the mint gate) | `postcondition not satisfied` |
| `device_assignment_preservation.rs` | drop the `sweeps_holder` hypothesis from `destroy_preserves` (the teardown sweep) | `postcondition not satisfied` |
| `device_assignment_preservation.rs` | make the sweep clear **every** device rather than the holder's | `sweep_is_exact`: `postcondition not satisfied` — the over-reach direction, which no invariant would have caught |
| `noninterference_instantiation.rs` **(rung 4a)** | blind `a_dev_view` to a bare taken/free bit (no `Held(h)` arm) | `step_consistent_holds`: `assertion failed` — a two-valued device bit cannot separate held-by-a-domain-the-caller-controls (idempotent `Ok`) from held-by-a-stranger (`Busy`) |
| `noninterference_instantiation.rs` **(rung 4a)** | empty out `ObsPlus::peer_live` | `step_consistent_holds`: `postcondition not satisfied` at the `DeviceAssign` guard — the assignee's liveness is a third domain's bit. **This component was missing from this file until rung 4a** (the bridge has had it since ⑥/F1): every earlier guard reading a third domain's liveness wrote a component no third observer carries, so the omission was invisible until a transition whose *outcome* every domain observes. |
| `noninterference_instantiation.rs` **(rung 4a)** | drop the device sweep from the `Destroy` cascade | ⚠️ **still verifies — correctly.** The sweep's content is the *safety* invariant "no device names a dead domain", which is `device_assignment_preservation.rs`'s obligation (dropping it **there** does reject) and the enumerator's. Non-interference asks a different question, and a surviving assignment moves no observation the cascade had not already moved. The sweep is modeled here for fidelity — had it opened a channel, this is where that would have had to be accounted for. |
...
| `mislevelled_link_preservation.rs` (I-2a) | drop the `+ 1` in `get_type_incr` (an acquire that takes no reference) | `postcondition not satisfied` (`get_type_incr_establishes`) |
| `mislevelled_link_preservation.rs` (I-2a) | drop the exclusivity precondition (let a `Writable` acquire fire with `pt > 0`) | `assertion failed` (`get_type_incr_monotone` — the add would flip a live type) |
| `mislevelled_link_preservation.rs` (I-2b) | drop the `count_positive` witness in a bridge (e.g. `parent_keeps_type`) | `postcondition not satisfied` (the count is no longer forced `≥ 1`) |
| `mislevelled_link_preservation.rs` (I-2b) | claim a decrement leaves a count unchanged (e.g. `unlink`'s parent `pt`, drop the `- 1`) | `postcondition not satisfied` (`accounted` fails to hold after) |
