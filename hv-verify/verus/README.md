...
| `mislevelled_link_preservation.rs` **(the metal refinement — Phase I-2a)** | **`MislevelledLink` preserved, ∀-N — the page-table *shape* half.** The standing hierarchy invariant (every live edge's parent is a page table and its child is exactly the reference the entry's shape demands) is preserved by its **additive** transitions, proven outright at arbitrary edge and frame count: `p2m_link` (the up-front table check + two `get_type` acquires **establish** the new edge, and an acquire cannot flip a live type — the one borrow from `TypeConfusion` exclusivity), `grant_map`/`pin` (adds-only), and `p2m_allocate` (**purely type-monotone — needs *no* borrow**, the instructive contrast with `foreign_link`'s ownership-reading allocate), plus the base case. The **decrement** transitions (`unlink`/`unlink_all`, `unpin`/`grant_unmap`, `free`, `DomainDestroy`) are stated with a single named coupling `type_preserved_on` (`TypeRefsAccounted`) as hypothesis: *when a holder leaves, every frame still referenced by a surviving edge keeps its type.* This is the p2m cousin of `RefcountMismatch` and is **not** currently a checked invariant (it holds by construction of the balanced `get`/`put` discipline) — **deferred to I-2b**, which discharges it and closes the residual. *Finding: for `MislevelledLink`, `DomainDestroy`'s `has_foreign_link_into` precondition **is** load-bearing (a surviving foreign edge into a freed frame would break it) — cashing the localization `foreign_link_preservation.rs` recorded.* | the metal — `docs/STAGE2-REFINEMENT-FORALL-N.md` |
...
| `mislevelled_link_preservation.rs` | drop the `+ 1` in `get_type_incr` (an acquire that takes no reference) | `postcondition not satisfied` (`get_type_incr_establishes`) |
| `mislevelled_link_preservation.rs` | drop the exclusivity precondition (let a `Writable` acquire fire with `pt > 0`) | `assertion failed` (`get_type_incr_monotone` — the add would flip a live type) |
